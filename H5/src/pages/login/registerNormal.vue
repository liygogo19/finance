<template>
    <f7-page  id="register-normal">
        <f7-navbar title="注册" back-link="Back" sliding></f7-navbar>
        <f7-block>
            <f7-list form id="register-normal" class="without-border without-margintop">
                <f7-list-item class="slogan-title item-16">
                    此为负责人专属注册链接
                </f7-list-item>
                <!-- Text Input -->
                ...
                <f7-list-item :class="{ 'form-group--error': $v.company_name.$error }">
                    <f7-input class="with-icon form_bg company" type="text" v-model.lazy.trim="company_name" @input="$v.company_name.$touch()" placeholder="请填写公司名字, 如优米房产"/>
                    <div v-if="$v.company_name.$dirty">
                        <span class="form-group__message" v-if="!$v.company_name.required">* 公司名字不能为空</span>
                    </div>
                </f7-list-item>
                ...
                <f7-block>
                    <f7-button big class="font-16" :class="{ disabled: disabledButton }" @click="registerNormal">注 册</f7-button>
                </f7-block>
            </f7-list>
        </f7-block>
    </f7-page>
...
<script>
    ...
    export default {
        data(){
            return {
                key:'b00184378bcbe0f8f414b93504fe4442',
                name:'',
                sex:'',
                phone:'',
                captcha:'',
        ...
        methods:{
            ...
            onF7Init:function () {
                this.f7_mobile_widgets.f7_mobile_captcha(this,'#send-code-normal');
        ...
        mounted(){
            this.$store.dispatch('beforeRegister',{'ref':this.$route.params.leader_ref});
